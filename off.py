...
async def main():
    ...
    async def _turn_off(sdk):
        if not args.json:
            print(f"Turning off bulb {sdk.mac_address} (Fade: {fade_ms}ms)...")
        async with sdk:
            await sdk.turn_off(transit=fade_ms)

    async def _mesh_turn_off(sdk):
        if not args.json:
            print(f"[{sdk.mac_address}] Sending mesh proxy turn_off to {len(targets)} targets (Fade: {fade_ms}ms, Retries: {args.retries})...")
        async with sdk:
            for i in range(args.retries):
                await sdk.turn_off(transit=fade_ms, targets=targets)
                if i < args.retries - 1:
                    await asyncio.sleep(0.3)
        
    async def _stream_turn_off():
        if not args.json:
            print(f"Streaming turn_off to {len(targets)} targets (Fade: {fade_ms}ms)...")
        ...
        try:
            ...
            if active_sdks:
                outcomes = await asyncio.gather(*(sdk.turn_off(transit=fade_ms) for sdk in active_sdks), return_exceptions=True)
